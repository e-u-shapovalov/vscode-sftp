...
`filePerm` и `dirPerm` принимают обычный восьмеричный режим вроде `644` или `755` и применяются как к выгружаемым файлам, так и к элементам, созданным в дереве; значение, не являющееся 1–4 восьмеричными цифрами, игнорируется, и остаётся default сервера. Мастер настройки сразу вписывает `644` / `755` в новый конфиг.
...
Не коммитьте учётные данные открытым текстом. При создании конфигурации мастер даёт выбрать между SSH-ключом, хранилищем ОС (`secretStorage`) и открытым текстом:

![Мастер настройки WireFerry выбирает, как хранить пароль: SSH-ключ, хранилище ОС или открытый текст](../assets/showcase/setup-wizard-password-storage.png)
...
| `wireferry.operationLog` | `tab` | Куда попадает журнал выгрузки/скачивания/удаления: `tab` (новая вкладка редактора), `output` (канал вывода WireFerry — без вкладки, без смены фокуса, панель не открывается) или `off`. В режимах `output` и `off` операция с ошибками всё равно даёт уведомление со сводкой |
