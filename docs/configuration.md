...
| `wireferry.operationLog` | `tab` | Where the upload/download/delete log goes: `tab` (new editor tab), `output` (WireFerry output channel — no tab, no focus change, panel not opened) or `off`. In `output` and `off` a run with failures still raises a notification with the tally |
