...
**English — in short**
- **2.7.1** — a folder-size fix. With "show folder sizes" or "sort by size" turned on, a **local-only** folder in the unified tree (on your disk, not on the server) kicked off an endless, always-failing server-side `du` — the tree flickered, the connection filled with failing requests, and other folders (like a `/root` you want to open as root) couldn't load. Local-only folders are now skipped by that measurement.
...
**Русский — коротко**
- **2.7.1** — фикс размеров папок. При включённых «показывать размеры папок» или «сортировать по размеру» **локальная-только** папка в объединённом дереве (есть на диске, нет на сервере) запускала бесконечный, всегда падающий серверный `du` — дерево дёргалось, соединение заваливалось запросами, а другие папки (например, `/root`, который хочется открыть от root) не грузились. Теперь такие папки в измерении размеров пропускаются.
...
## 2.7.1 — Folder-size retry-loop fix · Фикс цикла измерения размеров папок

**English:**

A bugfix for a regression visible only with the opt-in **`remoteExplorer.showSize`** or **`remoteExplorer.sortBySize`** settings enabled (both default off). In the unified local+server tree a **local-only** folder has no server path, so the background folder-size `du` always failed on it — and because a local-only node's cached size is reset on every render, that failure re-measured forever. The result: a flickering tree, an endless stream of failing `du` exec channels on the SSH connection, and other folders (e.g. a root-owned `/root`) starved of the connection so they never listed. Local-only folders are now excluded from the size measurement — it runs once for real server folders and stops.

**Русский:**

Фикс регрессии, заметной только при включённых опциональных настройках **`remoteExplorer.showSize`** или **`remoteExplorer.sortBySize`** (обе по умолчанию выключены). В объединённом дереве local+сервер у **локальной-только** папки нет серверного пути, поэтому фоновый `du` для размера всегда падал на ней — а так как кэш размера local-only узла сбрасывается на каждом ре-рендере, измерение повторялось бесконечно. Итог: дёрганье дерева, бесконечный поток падающих `du`-каналов на SSH-соединении и голодание других папок (например, root-овой `/root`), из-за чего они не листались. Теперь локальные-только папки исключены из измерения размеров — оно проходит один раз для реальных серверных папок и останавливается.
