...
**Русский — коротко**
- **2.9.1** — дерево сервера остаётся там, где вы его оставили. Удаление root-ового файла, переименование или перемещение, drag & drop, `chmod -R`, «Выгрузить изменённые» и применение локального переименования на сервере — всё это раньше заканчивалось полным обновлением дерева: каждая раскрытая папка перечитывалась с сервера, вид уезжал наверх, и после удаления одного файла на третьем уровне вложенности приходилось заново искать, где вы были. Теперь WireFerry перечитывает **только те папки, которые действительно изменились** — всё остальное сохраняет и свои листинги, и раскрытое состояние, — а множественный выбор из одной папки стоит **одного** листинга вместо одного на каждый элемент. Полное обновление осталось там, где дерево и правда меняется целиком: кнопка «Обновить» на панели, смена профиля, сохранение конфигурации и создание нового сервера. Рекурсивный `chmod` вдобавок сбрасывает кэшированные пометки «только чтение» ниже по дереву, чтобы свёрнутая ветка не показывала бейдж по правам, которых уже нет.
- **2.9.0** — теперь можно создавать файлы и папки в каталогах, которые вам не принадлежат. ПКМ по `/etc` (или любому root-овому пути) → **Создать файл**, и вместо прежнего тупика «Permission denied» WireFerry предложит **«Создать от root»**: запись выполняется через `su` с тем же одним паролем на окно, что и остальные root-действия, применяются ваши `filePerm`/`dirPerm`, а всё, что уже существует по этому пути, не трогается — существующий файл или симлинк никогда не обнуляется и не перезаписывается «сквозь ссылку». Создание было последней операцией записи без такой возможности: у выгрузки, скачивания, удаления, chmod/chown и листинга она уже была. Новый файл вдобавок **сразу открывается локально** — можно начинать печатать, а не искать его в дереве; сохранение назад идёт через уже существующее «применить от root». Если файл с таким именем уже лежит у вас на диске, WireFerry честно скажет об этом и переспросит перед открытием: на сервер при первом сохранении уедет именно это содержимое, а не пустой файл. При создании теперь виден индикатор, так что на медленном канале окно больше не выглядит зависшим.
- **2.8.4** — вы сами решаете, куда девать журнал операций, а расширение делает заметно меньше лишней работы. Новая настройка **`wireferry.operationLog`** (issue #3) отправляет журнал выгрузки/скачивания/удаления в **новую вкладку** (по умолчанию, как и раньше), в **канал вывода WireFerry** (без вкладки, без кражи фокуса, панель тоже сама не всплывает) или **никуда** — при этом операция с ошибками всё равно даёт уведомление с кнопкой **«Показать журнал»**, так что отключение журнала никогда не заглушает сбой. Под капотом: передача больше не спрашивает у сервера размер каждого файла, когда уже знает его (на FTP это был полный листинг папки на каждый файл), сохранение файла больше не выбрасывает кэш всего дерева, а вотчер выгружает пачку по несколько файлов сразу, а не по одному. Два из этих мест оказались настоящими багами — **«Выгрузить изменённые» могла молча недосчитаться файлов**, потому что каждое сохранение сбрасывало очередь фоновых проверок, а пачка вотчера могла **молча потерять правку**, посчитав файл всё ещё скачивающимся. Отрисовка дерева тоже подешевела (настройки и язык больше не перечитываются на каждую строку), причём порядок сортировки остался ровно прежним.
...
## 2.9.1 — targeted tree refresh · точечное обновление дерева

**English**
- **Only the changed folders are re-listed.** Six operations ended with a tree-wide refresh that dropped every cached listing and rebuilt the root level: delete-as-root, rename/move, in-tree drag & drop, recursive `chmod`, "Upload Modified", and applying a local rename on the server. Each of them knows exactly which paths it touched, so each now hands those paths to the explorer and only their parent folders are re-listed. Everything outside them keeps its cached listings — and the user keeps their place in the tree.
- **One listing per folder, not per item.** The new batch entry point (`refreshParentsOf`) invalidates every touched path first and only then signals the view, de-duplicating by parent: deleting five files from one folder now costs one server listing instead of five, and a move signals the source folder and the destination folder exactly once each.
- **A folder that isn't on screen isn't fetched.** The batch resolves parents from the tree's own node cache rather than materialising them, so re-listing never pulls a directory the view isn't showing. Its cached listing was dropped all the same, so it re-reads from the server the moment it is expanded.
- **A path the tree can't place still forces a full refresh** — a config that has been removed or fails validation falls back to re-reading everything rather than leaving a deleted entry on screen. The refresh helper never throws: it is called from `finally` blocks and from VS Code's own rename handler.
- **A recursive `chmod` also clears the cached write hints below the target.** The re-listing only reaches folders the tree actually re-reads, so a collapsed branch would otherwise keep the read-only badge — and feed "Upload Modified" a stale `writable` — for permissions that had just been replaced.
- Internals: the targeted half of `refresh()` is extracted into `_invalidateTargeted`, leaving the full-refresh branch byte-for-byte as it was; the preview-document signal is no longer sent for entries the tree knows to be directories.

**Русский**
- **Перечитываются только изменившиеся папки.** Шесть операций заканчивались обновлением всего дерева, которое сбрасывало каждый закэшированный листинг и пересоздавало корневой уровень: удаление от root, переименование/перемещение, drag & drop внутри дерева, рекурсивный `chmod`, «Выгрузить изменённые» и применение локального переименования на сервере. Каждая из них точно знает, какие пути затронула, — теперь она передаёт эти пути дереву, и перечитываются только их родительские папки. Всё, что вне их, сохраняет свои листинги, а пользователь — своё место в дереве.
- **Один листинг на папку, а не на элемент.** Новая batch-точка входа (`refreshParentsOf`) сначала инвалидирует все затронутые пути и только потом сообщает представлению, дедуплицируя по родителю: удаление пяти файлов из одной папки стоит одного серверного листинга вместо пяти, а перемещение ровно по одному разу трогает папку-источник и папку-назначение.
- **Папка, которой нет на экране, не запрашивается.** Родители ищутся в собственном кэше узлов дерева, а не материализуются, поэтому обновление никогда не тянет с сервера каталог, который не отображается. Его закэшированный листинг всё равно сброшен — он перечитается в момент раскрытия.
- **Путь, который дерево не может разместить, по-прежнему вызывает полное обновление** — удалённый или не проходящий валидацию конфиг приводит к перечитыванию всего, лишь бы не оставить удалённую запись на экране. Сам помощник обновления не бросает исключений: его вызывают из блоков `finally` и из обработчика переименования самого VS Code.
- **Рекурсивный `chmod` вдобавок сбрасывает кэшированные пометки прав ниже цели.** Перечитывание доходит только до папок, которые дерево реально перезапрашивает, поэтому свёрнутая ветка иначе сохраняла бы бейдж «только чтение» — и подсовывала бы «Выгрузить изменённые» устаревший `writable` — для прав, которые только что заменили.
- Внутреннее: таргетная половина `refresh()` вынесена в `_invalidateTargeted`, ветка полного обновления осталась в точности прежней; сигнал документа-предпросмотра больше не отправляется для записей, о которых дерево знает, что это папки.

---

## 2.9.0 — create in root-owned directories · создание в каталогах, принадлежащих root

**English**
- **Create as root.** "New File" / "New Folder" in a directory the login user can't write (`/etc`, `/usr/local`, …) used to end at a `Permission denied` toast: the create went straight to the SFTP session and had no escalation path, because it never builds a `TransferTask` and so never reaches the `afterTransfer` hook the save-side recovery hangs on. It now offers **Create as root** and runs the create over `su`, reusing the window-cached root password. The entry is owned by `root:root` (as with upload-as-root) and gets `filePerm`/`dirPerm` applied, defaulting to 644/755.
- **An occupied path is never touched.** The script tests the target before creating (`-e` and `-L`, so a dangling symlink counts too) and creates the file with noclobber — `O_CREAT|O_EXCL` — which the kernel refuses on both an existing path and a symlink. So a name that appears between the check and the write fails the create instead of truncating a file or writing through a link to somewhere else entirely.
- **A failed `chmod` no longer reports a failed create.** The entry exists at that point, so the fallback warns that the mode didn't stick and still reveals the new entry — matching how the ordinary create treats the same error.
- **A new file opens locally right away**, so it can be filled in without a second trip to the tree; saving it back uses the existing "apply as root" recovery. If a non-empty local file already carries that name, WireFerry asks before opening it: what would open is that older local content, and the first save would upload it over the empty file just created on the server.
- **`createFile` / `createDir` now report whether they created anything.** "Already exists" used to resolve as success, which — with the new open step — would have opened an empty local copy over an existing remote file and wiped it on the next save.
- **Creating shows progress.** Several server round trips (stat → write → chmod → re-list the parent) previously ran with no visible sign, which on a slow link reads as a frozen window.
- Review rounds also fixed: a busy elevation-dialog gate no longer ends the command in complete silence; the `downloadOnOpen` suppression is now armed immediately before the document is shown rather than before a dialog that may outlive its three-second window (it could otherwise pull the empty server file over the local copy the user had just chosen to keep).

**Русский**
- **Создание от root.** «Создать файл» / «Создать папку» в каталоге, куда login-пользователь писать не может (`/etc`, `/usr/local`, …), раньше упиралось в тост `Permission denied`: создание шло напрямую в SFTP-сессию и не имело пути эскалации, потому что не создаёт `TransferTask` и не доходит до хука `afterTransfer`, на котором держится восстановление при сохранении. Теперь предлагается **«Создать от root»**, и запись выполняется через `su` с тем же паролем, что уже закэширован на окно. Объект принадлежит `root:root` (как и при загрузке от root) и получает `filePerm`/`dirPerm`, по умолчанию 644/755.
- **Занятый путь не трогается.** Скрипт проверяет цель перед созданием (`-e` и `-L`, то есть и «висячий» симлинк), а сам файл создаёт с noclobber — `O_CREAT|O_EXCL`, — что ядро отклоняет и для существующего пути, и для символической ссылки. Поэтому имя, появившееся между проверкой и записью, приводит к отказу, а не к обнулению файла или записи «сквозь ссылку» совсем в другое место.
- **Сбой `chmod` больше не выдаётся за сбой создания.** Объект к этому моменту уже создан, поэтому фолбэк предупреждает, что права не выставились, и всё равно показывает новую запись в дереве — ровно так же, как обычное создание относится к той же ошибке.
- **Новый файл сразу открывается локально**, чтобы его можно было наполнить, не возвращаясь в дерево; сохранение назад идёт через существующее «применить от root». Если локально уже лежит непустой файл с таким именем, WireFerry переспросит перед открытием: откроется именно то, старое локальное содержимое, и первое же сохранение выгрузит его поверх только что созданного на сервере пустого файла.
- **`createFile` / `createDir` теперь сообщают, создали ли они что-нибудь.** Раньше «уже существует» завершалось как успех — вместе с новым шагом открытия это означало бы пустую локальную копию поверх существующего серверного файла и его затирание при следующем сохранении.
- **При создании виден индикатор.** Несколько обращений к серверу (stat → запись → chmod → перечитывание родительской папки) раньше шли без единого признака жизни, и на медленном канале это выглядело как зависшее окно.
- По итогам ревью также исправлено: занятый гейт диалогов повышения прав больше не завершает команду полной тишиной; супрессия `downloadOnOpen` теперь взводится непосредственно перед показом документа, а не перед диалогом, который может пережить её трёхсекундное окно (иначе пустой серверный файл мог перезаписать локальную копию, которую пользователь только что решил сохранить).

---

## 2.8.4 — where the operation log goes, and a speed pass · куда попадает журнал операций, и работа над скоростью
...
- **A save no longer re-measures every folder in the root.** Cached folder sizes and symlink targets were dropped for the whole tree on any refresh, so with sort-by-size on, saving one file sent a server-side `du` over every folder at the root. A targeted refresh now clears them for the affected subtree and its ancestors — which genuinely did change size — and leaves sibling branches alone.
...
- **Сохранение файла больше не пересчитывает размер всех папок корня.** Кэш размеров папок и целей симлинков сбрасывался по всему дереву при любом обновлении, поэтому при включённой сортировке по размеру сохранение одного файла запускало серверный `du` по всем папкам корня. Точечное обновление теперь чистит их только для затронутого поддерева и его предков — у которых размер действительно изменился, — а соседние ветки не трогает.
