...
Последний релиз перестраивает контекстные меню так, чтобы команды WireFerry было легко отличить от встроенных в VS Code. В локальном проводнике и в меню вкладки редактора служебные команды (Config, Показать в Remote Explorer, Показать дерево, Размер и MD5, Diff с сервером, Удаление сервер/локально/оба) собраны в одно подменю **WireFerry ▸**, а повседневные **Синхронизация / Выгрузка / Скачивание** остались на верхнем уровне, в один клик. «Копировать путь (Git Bash)» остался рядом с нативным Copy Path, а в теле редактора все команды WireFerry идут одним блоком. Это изменение только в меню — ничего не переименовано и не удалено, каждая команда доступна там же, где и была.
