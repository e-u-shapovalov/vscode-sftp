...
- Читать отчёт об операции после каждой передачи или удаления по правому клику — вкладку `upload.log`, `download.log` или `delete.log` со списком файлов, их размером, датой и правами.
...
Последний релиз доводит объединённое дерево «локально + сервер» до боевого состояния и завершает доступ от root. **Статус M** теперь отражает реальное содержимое: для двух файлов одного размера WireFerry сверяет **MD5 в фоне**, прежде чем ставить `M`, — простое расхождение по времени не помечается как изменение, а **папки-предки наследуют `M`**, так что невыгруженное изменение видно сразу. Появилась возможность **скачать или править root-овый файл, который вы не можете прочитать**, и **загрузить папку в путь без прав записи (например `/root`) от root** — со стейджем во временный каталог и переносом на место через `su`. Новые файлы и папки, **созданные в дереве**, по умолчанию с безопасными **644/755** (выгрузка сохраняет режим локального файла, если `filePerm` не задан), а существующему конфигу при старте **дописываются `filePerm` / `dirPerm`**, чтобы настройка была видна и настраиваема (issue #2). Плюс фиксы объединённого дерева: больше нет цикла измерения размеров на локальных-только папках, нет `Config Not Found` после удаления в дереве, а локальные-только строки не предлагают серверные действия.
