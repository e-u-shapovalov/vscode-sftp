...
- Перемещать файлы и папки на сервере перетаскиванием прямо в дереве Remote Explorer.
...
Последний релиз добавляет исправления надёжности по итогам код-ревью — явное предупреждение, когда при удалении с сервера и локально локальный файл остаётся, и полное обновление дерева после рекурсивного `chmod -R` — плюс большую доработку документации и выравнивание JSON-схемы. Недавняя команда **«Показать дерево»** строит ASCII-дерево папки на сервере или локально из контекстного меню папки.
