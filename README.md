...
The latest release makes the unified local + server tree production-solid and completes root access. **Modified (M)** now reflects real content: for two same-size files WireFerry verifies **MD5 in the background** before flagging `M`, so a mere timestamp difference is not shown as modified, and **parent folders inherit `M`** so you can spot an un-uploaded change at a glance. You can now **download or edit a root-owned file you can't read**, and **upload a folder into a write-protected path (e.g. `/root`) as root** — staged to a temp dir, then moved into place via `su`. New files and folders **created in the tree** default to safe **644/755** (an upload keeps the local file's mode unless `filePerm` is set), and an existing config gets **`filePerm` / `dirPerm` written in** on startup so the setting is visible and tuneable (issue #2). Plus fixes to the unified tree: no more folder-size retry loop on local-only folders, no `Config Not Found` after a tree delete, and local-only rows no longer offer server-only actions.
